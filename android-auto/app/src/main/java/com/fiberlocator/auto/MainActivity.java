...
public class MainActivity extends Activity {
    ...
    private String mapHtml(Ticket focus) {
        ...
        return "<!doctype html><html><head><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\">"
            ...
            + "function showPhotoSheet(p){const url=photoUrl(p);const title=String(p.original_name||p.location_label||p.ticket||'Location photo');const detail=[p.ticket,p.location_label,p.address].filter(Boolean).join(' - ');const sheet=document.getElementById('photoSheet');sheet.innerHTML=`<strong>${esc(title)}</strong><img src=\"${esc(url)}\" alt=\"\"><small>${esc(detail||'Photo location')}</small><div class=\"photo-sheet-actions\"><a href=\"${esc(url)}\" target=\"_blank\">Open full</a><button type=\"button\" id=\"photoSheetEdit\">Edit / add photos</button><button type=\"button\" id=\"photoSheetClose\">Close</button></div>`;sheet.classList.add('open');document.getElementById('photoSheetClose').onclick=()=>sheet.classList.remove('open');document.getElementById('photoSheetEdit').onclick=()=>{try{FiberLocator.openLocationPhotos();}catch(e){}};}function addLocationPhotoMarkers(list){(Array.isArray(list)?list:[]).forEach(p=>{const lat=Number(p.lat),lng=Number(p.lng);if(!Number.isFinite(lat)||!Number.isFinite(lng))return;const marker=L.circleMarker([lat,lng],{pane:'photoPane',renderer:photoRenderer,radius:5,color:'#0f172a',fillColor:'#f8fafc',opacity:.5,fillOpacity:.42,weight:1.3,interactive:true,bubblingMouseEvents:false}).addTo(photoLayer);marker.on('click',e=>{if(e.originalEvent)L.DomEvent.stop(e.originalEvent);showPhotoSheet(p);});});}"
