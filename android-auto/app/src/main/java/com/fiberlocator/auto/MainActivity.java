...
public class MainActivity extends Activity {
    ...
    private void selectNav(Button selected) {
        ...
        ticketsNav.setTypeface(null, selected == ticketsNav ? Typeface.BOLD : Typeface.NORMAL);
        mapNav.setTypeface(null, selected == mapNav ? Typeface.BOLD : Typeface.NORMAL);
        menuNav.setTypeface(null, selected == menuNav ? Typeface.BOLD : Typeface.NORMAL);
    }
    ...
    private void showTicketDetail(Ticket ticket) {
        ...
        LinearLayout photoSection = column(0, dp(8), 0, dp(8));
        photoSection.addView(text("Location photos", 17, ink, true));
        photoSection.addView(text("Loading photos attached to this ticket...", 13, muted, false));
        page.addView(photoSection);
        loadTicketLocationPhotos(ticket, photoSection);
    ...
    private void loadTicketLocationPhotos(Ticket ticket, LinearLayout section) {
        executor.execute(() -> {
            try {
                List<LocationPhoto> allPhotos = repository.loadLocationPhotos();
                List<LocationPhoto> matches = new ArrayList<>();
                for (LocationPhoto photo : allPhotos) {
                    if (ticket.ticketNumber.equals(photo.ticket)) matches.add(photo);
                }
                runOnUiThread(() -> renderTicketLocationPhotos(ticket, section, matches));
            } catch (Exception error) {
                runOnUiThread(() -> {
                    section.removeAllViews();
                    section.addView(text("Location photos", 17, ink, true));
                    section.addView(text(error.getMessage() == null ? "Unable to load ticket photos." : error.getMessage(), 13, danger, false));
                });
            }
        });
    }

    private void renderTicketLocationPhotos(Ticket ticket, LinearLayout section, List<LocationPhoto> photos) {
        section.removeAllViews();
        section.addView(text("Location photos", 17, ink, true));
        if (photos.isEmpty()) {
            section.addView(text("No location photos are attached to this ticket yet.", 13, muted, false));
            return;
        }
        for (LocationPhoto photo : photos) {
            Button button = secondaryButton(photoButtonLabel(photo));
            button.setGravity(Gravity.CENTER_VERTICAL | Gravity.START);
            button.setOnClickListener(view -> showLocationPhotoViewer(ticket, photo));
            section.addView(button, new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, dp(52)));
        }
    }

    private String photoButtonLabel(LocationPhoto photo) {
        String title = photo.originalName.isEmpty() ? "Location photo" : photo.originalName;
        String detail = photo.locationLabel.isEmpty() ? photo.address : photo.locationLabel;
        if (detail == null || detail.trim().isEmpty()) detail = photo.hasCoordinates() ? String.format(java.util.Locale.US, "%.6f, %.6f", photo.latitude, photo.longitude) : photo.uploadedAt;
        return title + (detail == null || detail.trim().isEmpty() ? "" : "\n" + detail);
    }

    private void showLocationPhotoViewer(Ticket ticket, LocationPhoto photo) {
        screen = "detail";
        activeTicket = ticket;
        activeTicketNumber = ticket.ticketNumber;
        chrome.setVisibility(View.VISIBLE);
        if (nav != null) nav.setVisibility(View.VISIBLE);
        title.setText(photo.originalName.isEmpty() ? "Location photo" : photo.originalName);
        subtitle.setText(ticket.title());
        content.removeAllViews();
        LinearLayout page = column(dp(10), dp(10), dp(10), dp(10));
        Button back = secondaryButton("Back to ticket");
        back.setOnClickListener(view -> showTicketDetail(ticket));
        page.addView(back, new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, dp(46)));
        ...
        String url = absoluteDashboardUrl(photo.url);
        String html = "<!doctype html><html><head><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><style>html,body{margin:0;height:100%;background:#0f172a}body{display:grid;place-items:center;overflow:auto}img{max-width:100vw;max-height:100vh;width:auto;height:auto;object-fit:contain}</style></head><body><img src=\"" + escapeHtmlAttribute(url) + "\"></body></html>";
        web.loadDataWithBaseURL(AppSettings.dashboardUrl(this), html, "text/html", "UTF-8", null);
        page.addView(web, new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, 0, 1));
        content.addView(page);
    }

    private String absoluteDashboardUrl(String path) {
        String clean = path == null ? "" : path.trim();
        if (clean.startsWith("http://") || clean.startsWith("https://")) return clean;
        if (!clean.startsWith("/")) clean = "/" + clean;
        return AppSettings.dashboardUrl(this) + clean;
    }
    ...
    private String mapHtml(Ticket focus) {
        ...
        return "<!doctype html><html><head><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\">"
            ...
            + "map.createPane('photoPane');map.getPane('photoPane').style.zIndex=675;map.getPane('photoPane').style.pointerEvents='auto';const photoRenderer=L.canvas({pane:'photoPane'});const photoLayer=L.layerGroup().addTo(map);map.createPane('featurePane');map.getPane('featurePane').style.zIndex=650;map.getPane('featurePane').style.pointerEvents='auto';const featureRenderer=L.canvas({pane:'featurePane'});"
            ...
            + "const TILE_STYLES={standard:{url:'https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',subdomains:'abc',maxZoom:20},contrast:{url:'https://{s}.basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}{r}.png',subdomains:'abcd',maxZoom:20},detailed:{url:'https://server.arcgisonline.com/ArcGIS/rest/services/World_Street_Map/MapServer/tile/{z}/{y}/{x}',maxZoom:20},light:{url:'https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png',subdomains:'abcd',maxZoom:20},dark:{url:'https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png',subdomains:'abcd',maxZoom:20},terrain:{url:'https://{s}.tile.opentopomap.org/{z}/{x}/{y}.png',subdomains:'abc',maxZoom:17},satellite:{url:'https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}',maxZoom:20},hybrid:{layers:['https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}','https://server.arcgisonline.com/ArcGIS/rest/services/Reference/World_Transportation/MapServer/tile/{z}/{y}/{x}','https://server.arcgisonline.com/ArcGIS/rest/services/Reference/World_Boundaries_and_Places/MapServer/tile/{z}/{y}/{x}'],maxZoom:20},'mapbox-streets':{provider:'mapbox',styleId:'streets-v12'},'mapbox-outdoors':{provider:'mapbox',styleId:'outdoors-v12'},'mapbox-standard':{provider:'mapbox',styleId:'standard'},'mapbox-standard-satellite':{provider:'mapbox',styleId:'standard-satellite'},'mapbox-light':{provider:'mapbox',styleId:'light-v11'},'mapbox-dark':{provider:'mapbox',styleId:'dark-v11'},'mapbox-satellite':{provider:'mapbox',styleId:'satellite-v9'},'mapbox-satellite-streets':{provider:'mapbox',styleId:'satellite-streets-v12'},'mapbox-satellite-traffic':{provider:'mapbox',styleId:'satellite-streets-v12'},'mapbox-navigation-day':{provider:'mapbox',styleId:'navigation-day-v1'},'mapbox-navigation-night':{provider:'mapbox',styleId:'navigation-night-v1'},'mapbox-traffic-day':{provider:'mapbox',styleId:'traffic-day-v2'},'mapbox-traffic-night':{provider:'mapbox',styleId:'traffic-night-v2'}};"
            ...
            + "function addr(f){return prop((f&&f.properties)||{},'Street Address','Street_Address','street_address','Service Address','service_address','Customer Address','customer_address','full_address','Address','address');}"
            + "function slLabel(f){return addr(f)||prop(f.properties,'ID','feature_id')||'SL';}"
            ...
            + "function entries(p){return Object.keys(p||{}).filter(k=>p[k]!=null&&String(p[k]).trim()).map(k=>[k,String(p[k])]);}"
            + "function popupHtml(kind,f){const p=f.properties||{};const id=kind==='vetro'?vetroId(f):vitId(f);const title=addr(f)||prop(p,'ID','feature_id','Name','name','label','vetro_id','vitruvi_id')||(kind==='vetro'?'VETRO feature':'Vitruvi feature');let used={};['Street Address','Street_Address','street_address','Service Address','service_address','Customer Address','customer_address','full_address','Address','address','ID','feature_id','Name','name','label','vetro_id','vitruvi_id','layer_id','Layer_ID','status_id','Status','status','plan','plan_id','Note','note'].forEach(k=>used[k.toLowerCase()]=1);let rows=`<div><strong>Layer:</strong> ${esc(id)}</div>`;const curated=[['Address',addr(f)],['Feature ID',prop(p,'ID','feature_id','vetro_id','vitruvi_id','id','uid')],['Name',prop(p,'Name','name','label')],['Status',prop(p,'status_id','Status','status')],['Plan',prop(p,'plan','plan_id')],['Note',prop(p,'Note','note')]];curated.forEach(r=>{if(r[1])rows+=`<div><strong>${esc(r[0])}:</strong> ${esc(r[1])}</div>`;});entries(p).forEach(r=>{if(!used[String(r[0]).toLowerCase()])rows+=`<div><strong>${esc(r[0])}:</strong> ${esc(r[1])}</div>`;});return `<div class=\"feature-popup\"><strong>${esc(title)}</strong>${rows}</div>`;}"
            + "function labelFeature(kind,f){if(kind!=='vetro'||!addr(f))return false;const id=vetroId(f);return id==='prefix:SL'||id==='26'||/customer|service/i.test(String(id));}"
            + "let featureGroups=[];function updateAddressLabels(){const show=map.getZoom()>=18;featureGroups.forEach(g=>g.eachLayer(l=>{if(!l.getTooltip||!l.getTooltip())return;if(show)l.openTooltip();else l.closeTooltip();}));}map.on('zoomend',updateAddressLabels);"
            ...
            + "map.on('click',e=>{if(addingNote){addMapNote(e.latlng);return;}if(measuring){addMeasurePoint(e.latlng);return;}const nearest=nearestPhotoFromTap(e.latlng);if(nearest)showPhotoSheet(nearest);});"
            ...
            + "function photoUrl(p){const id=String(p&&p.id||'');return String(p&&p.url||('/api/location-photos/file?id='+encodeURIComponent(id)));}"
            + "function photoPreviewUrl(p){const id=String(p&&p.id||'');return String(p&&p.thumbnail_url||p&&p.thumbnailUrl||p&&p.url||('/api/location-photos/thumb?id='+encodeURIComponent(id)));}"
            + "let locationPhotos=[];let photoCarousel=[];let photoCarouselIndex=0;"
            + "function photoLatLng(p){const lat=Number(p&&p.lat),lng=Number(p&&p.lng);return Number.isFinite(lat)&&Number.isFinite(lng)?L.latLng(lat,lng):null;}"
            + "function photoTitle(p){return String(p&&p.original_name||p&&p.location_label||p&&p.ticket||'Location photo');}"
            + "function photoDetail(p){return [p&&p.ticket,p&&p.location_label,p&&p.address].filter(Boolean).join(' - ');}"
            + "function setPhotoCarousel(anchor){const base=photoLatLng(anchor);const padded=map.getBounds().pad(.18);let visible=locationPhotos.filter(p=>{const ll=photoLatLng(p);return ll&&padded.contains(ll);});if(!visible.length)visible=locationPhotos.slice();visible.sort((a,b)=>map.distance(base||photoLatLng(a),photoLatLng(a))-map.distance(base||photoLatLng(b),photoLatLng(b)));photoCarousel=visible.slice(0,36);photoCarouselIndex=Math.max(0,photoCarousel.findIndex(x=>String(x.id)===String(anchor&&anchor.id)));}"
            + "function showPhotoOffset(delta){if(!photoCarousel.length)return;photoCarouselIndex=(photoCarouselIndex+delta+photoCarousel.length)%photoCarousel.length;showPhotoSheet(photoCarousel[photoCarouselIndex],true);}"
            + "function showPhotoSheet(p,reuse){if(!reuse)setPhotoCarousel(p);p=photoCarousel[photoCarouselIndex]||p;const url=photoUrl(p);const preview=photoPreviewUrl(p);const title=photoTitle(p);const detail=photoDetail(p);const count=photoCarousel.length?`${photoCarouselIndex+1} of ${photoCarousel.length}`:'';const sheet=document.getElementById('photoSheet');sheet.innerHTML=`<div class=\"photo-sheet-head\"><div><strong>${esc(title)}</strong><small>${esc([detail,count].filter(Boolean).join(' - '))}</small></div><button type=\"button\" id=\"photoSheetClose\" class=\"photo-close\">Close</button></div><div class=\"photo-stage\"><img src=\"${esc(preview)}\" alt=\"\"><button type=\"button\" id=\"photoPrev\" class=\"photo-nav prev\" aria-label=\"Previous photo\">&#8249;</button><button type=\"button\" id=\"photoNext\" class=\"photo-nav next\" aria-label=\"Next photo\">&#8250;</button></div><div class=\"photo-sheet-actions\"><a href=\"${esc(url)}\" target=\"_blank\">Open full</a><button type=\"button\" id=\"photoSheetEdit\">Edit / add photos</button></div>`;sheet.classList.add('open');document.getElementById('photoSheetClose').onclick=()=>sheet.classList.remove('open');document.getElementById('photoSheetEdit').onclick=()=>{try{FiberLocator.openLocationPhotos();}catch(e){}};document.getElementById('photoPrev').disabled=photoCarousel.length<2;document.getElementById('photoNext').disabled=photoCarousel.length<2;document.getElementById('photoPrev').onclick=()=>showPhotoOffset(-1);document.getElementById('photoNext').onclick=()=>showPhotoOffset(1);}"
            + "function nearestPhotoFromTap(latlng){if(!locationPhotos.length)return null;const tap=map.latLngToContainerPoint(latlng);const limit=map.getZoom()>=17?42:34;let best=null,bestPx=limit;locationPhotos.forEach(p=>{const ll=photoLatLng(p);if(!ll)return;const pt=map.latLngToContainerPoint(ll);const dx=pt.x-tap.x,dy=pt.y-tap.y;const d=Math.sqrt(dx*dx+dy*dy);if(d<=bestPx){best=p;bestPx=d;}});return best;}"
            + "function addLocationPhotoMarkers(list){locationPhotos=(Array.isArray(list)?list:[]).filter(p=>photoLatLng(p));locationPhotos.forEach(p=>{const ll=photoLatLng(p);const marker=L.circleMarker(ll,{pane:'photoPane',renderer:photoRenderer,radius:8,color:'#0f172a',fillColor:'#f8fafc',opacity:.68,fillOpacity:.52,weight:1.6,interactive:true,bubblingMouseEvents:false}).addTo(photoLayer);marker.on('click',e=>{if(e.originalEvent)L.DomEvent.stop(e.originalEvent);showPhotoSheet(p);});});}"
            + "fetch('/api/location-photos',{credentials:'include'}).then(r=>r.ok?r.json():null).then(data=>addLocationPhotoMarkers(data&&data.photos)).catch(e=>console.log('location photos failed',e&&e.message));"
            ...
            + "const group=L.geoJSON({type:'FeatureCollection',features:f},{pane:'featurePane',renderer:featureRenderer,style:x=>{const id=kind==='vetro'?vetroId(x):vitId(x);const geom=x.geometry&&String(x.geometry.type)||'';const c=color(kind,id,kind==='vetro'?(state.vetroColor||'#2563eb'):'#f97316');const raw=num(kind==='vetro'?state.vetroLayerOpacityOverrides:state.vitruviLayerOpacityOverrides,id,kind==='vetro'?(Number(state.vetroOpacity)||.72):.82,0,1);const op=kind==='vetro'?raw*vetroOpacityScale:raw;const sz=num(kind==='vetro'?state.vetroLayerSizeOverrides:state.vitruviLayerSizeOverrides,id,kind==='vetro'?3:3,1,18);const st={color:c,fillColor:c,weight:geom.startsWith('Line')?sz:1,opacity:op,fillOpacity:op*.32,radius:sz,baseOpacity:raw};const sn=styleName(kind,id,geom);if(geom.startsWith('Line')){st.fillOpacity=0;if(sn==='dashed')st.dashArray='8 6';if(sn==='dotted')st.dashArray='2 6';}return st;},pointToLayer:(x,ll)=>{const id=kind==='vetro'?vetroId(x):vitId(x);const c=color(kind,id,kind==='vetro'?(state.vetroColor||'#2563eb'):'#f97316');const raw=num(kind==='vetro'?state.vetroLayerOpacityOverrides:state.vitruviLayerOpacityOverrides,id,kind==='vetro'?(Number(state.vetroOpacity)||.72):.82,0,1);const op=kind==='vetro'?raw*vetroOpacityScale:raw;const sz=num(kind==='vetro'?state.vetroLayerSizeOverrides:state.vitruviLayerSizeOverrides,id,8,4,28);const marker=L.marker(ll,{pane:'featurePane',zIndexOffset:8500,icon:icon(styleName(kind,id,'Point'),c,sz*2,op)});marker.options.baseOpacity=raw;return marker;},onEachFeature:(x,l)=>{l.bindPopup(popupHtml(kind,x),{maxWidth:340});if(labelFeature(kind,x)){l.bindTooltip(esc(addr(x)),{permanent:true,direction:'right',offset:[8,0],className:'vetro-address-label',opacity:1,interactive:false});}l.on('click',e=>{if(e.originalEvent)L.DomEvent.stop(e.originalEvent);if(addingNote){addFeatureNote(kind,x,e.latlng);return;}if(measuring){addMeasurePoint(e.latlng);return;}l.openPopup(e.latlng);});}}).addTo(map);featureGroups.push(group);updateAddressLabels();if(kind==='vetro'){vetroLayers.push(group);applyVetroOpacity();}}catch(e){}}"
            ...
            + "let map3dClickLayers={};function popup3d(kind,f){const p={...(f.properties||{})};Object.keys(p).filter(k=>String(k).startsWith('_')).forEach(k=>delete p[k]);return popupHtml(kind,{properties:p});}function bind3dFeatureClicks(kind){[`$" + "{kind}-3d-fill`,`$" + "{kind}-3d-line-solid`,`$" + "{kind}-3d-line-dashed`,`$" + "{kind}-3d-line-dotted`,`$" + "{kind}-3d-point-symbol`,`$" + "{kind}-3d-point-label`].forEach(id=>{if(map3dClickLayers[id]||!map3d.getLayer(id))return;map3dClickLayers[id]=1;map3d.on('click',id,e=>{const f=e.features&&e.features[0];if(!f)return;new mapboxgl.Popup({maxWidth:'340px'}).setLngLat(e.lngLat).setHTML(popup3d(kind,f)).addTo(map3d);});map3d.on('mouseenter',id,()=>map3d.getCanvas().style.cursor='pointer');map3d.on('mouseleave',id,()=>map3d.getCanvas().style.cursor='');});}"
    ...
    private final class MapBridge {
        ...
        @JavascriptInterface
        public void openLocationPhotos() {
            runOnUiThread(() -> showLocationPhotos());
        }
